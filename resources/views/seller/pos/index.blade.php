                                ...
                                <div class="" id="cart-details">
                                    <div class="aiz-pos-cart-list mb-4 mt-3 c-scrollbar-light">
                                        ...
                                        @if (Session::has('pos.cart'))
                                                            ...
                                                            <div class="col-auto w-60px">
                                                                <div
                                                                    ...
                                                                    <button class="btn col-auto btn-icon btn-sm fs-15"
                                                                        ...
                                                                        data-field="qty-{{ $key }}"
                                                                        @if ($stock->product->digital == 1) disabled @endif>
                                            ...
                                            @php
                                                $discount = $subtotal * (Session::get('pos.discount', 0) / 100);
                                    ...
                                    <div>
                                        ...
                                        <div class="d-flex justify-content-between fw-600 mb-2 opacity-70">
                                            ...
                                            <span>{{ single_price($discount) }} ({{ Session::get('pos.discount', 0) }}%)</span>
                                        </div>
                                        <div class="d-flex justify-content-between fw-600 mb-2 opacity-70">
...
@section('modal')
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-zoom" role="document">
            <div class="modal-content">
                <div class="modal-header bord-btm">
                    <h4 class="modal-title h6">Crear Nuevo Usuario</h4>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <div class=" row">
                                ...
                                <div class="col-sm-10">
                                    <textarea placeholder="{{ translate('Address') }}" id="address" name="address" class="form-control rounded-15p"
                                        required></textarea>
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                            <div class=" row">
                                ...
                                <div class="col-sm-10">
                                    <select class="form-control aiz-selectpicker rounded-15p" data-live-search="true"
                            ...
                            <div class=" row">
                                ...
                                <div class="col-sm-10">
                                    <input type="number" min="0" placeholder="{{ translate('Phone') }}"
                                        id="phone" name="phone" class="form-control rounded-15p" required>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-styled btn-base-3 rounded-25px"
                            data-dismiss="modal">Cerrar</button>
                        <button type="submit"
                            class="btn btn-primary btn-styled btn-base-1 rounded-25px">{{ translate('Save') }}</button>
                    </div>
                </form>
            </div>
        </div>
            ...
            <div class="modal-content" id="variants">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary btn-base-3"
                        data-dismiss="modal">{{ translate('Cerrar') }}</button>
                    <button type="button" onclick="oflinePayment()"
                        class="btn btn-base-1 btn-warning">{{ translate('Transferencia Bancaria') }}</button>
    ...
    <div id="offlin_payment" class="modal fade" role="dialog">
        <div class="modal-dialog modal-dialog-centered modal-dialog-zoom" role="document">
            <div class="modal-content">
                <div class="modal-header bord-btm">
                    <h4 class="modal-title h6">{{ translate('Información de transferencia bancaria') }}</h4>
                    <button type="button" class="close" data-dismiss="modal">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="form-group">
                        <div class="row">
                            <label class="col-sm-3 control-label" for="offline_payment_method">{{ translate('Banco emisor') }}</label>
                            <div class="col-sm-9">
                                <select name="bank" class="form-control" required>
                                    <option value="" selected disabled>-- Seleccionar banco --</option>
                                    @foreach($banks as $bank)
                                        <option value="{{ $bank->bank_name }}" class="text-dark">{{ $bank->bank_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary btn-base-3 rounded-25px"
                        data-dismiss="modal">{{ translate('Close') }}</button>
                    <button type="button" onclick="submitOrder('manual_payment_1')"
                        class="btn btn-styled btn-base-1 btn-success rounded-25px">{{ translate('Confirm') }}</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script type="text/javascript">
        ...
        $("#confirm-address").click(function() {
            ...
            $.ajax({
                ...
                method: "POST",
                url: "{{ config('app.url') . "/api/v2/seller/pos/create-user" }}",
                data: data,
                cache: false,
                contentType: false,
                processData: false,
        ...
        function submitOrder(payment_type) {
            ...
            var offline_bank = $('select[name=bank]').val();
