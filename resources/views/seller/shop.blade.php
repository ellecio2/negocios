        ...
        <div class="col-md-6">
            ...
            <div class="card">
                ...
                <div class="card-body">
                        ...
                        <table id="horario_tienda" class="table">
                            ...
                            <tbody>
                                @foreach ($businessWorkingHours as $workingHours)
                                    <tr>
                                        <th scope="row">
                                            {{ $diasSemanaspanis[array_search($workingHours->dia_semana, $diasSemana)] }}
                                        </th>
                                        ...
                                        <td class="text-center">
                                            @if ($workingHours->laborable)
                                                <div class="alert alert-success" role="alert">Activo</div>
                                            @else
                                                <div class="alert alert-danger" role="alert">Desactivado</div>
                                            @endif
                                        </td>
                                        <td class="text-center">
                                            <button type="button" class="btn btn-success btn-sm" data-toggle="modal"
                                                data-target="#editModal_{{ $workingHours->id }}">Editar</button>
                                        </td>
                                    </tr>

                                    <!-- Modal de editar -->
                                    <div class="modal fade" id="editModal_{{ $workingHours->id }}" tabindex="-1" role="dialog"
                                        ...
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                ...
                                                <form class="working-hours-form" data-id="{{ $workingHours->id }}"
                                                    action="{{ route('seller.business_working_hours.update', $workingHours->id) }}" method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    <div class="modal-body">
                                                        <div class="form-group">
                                                            <label for="hora_inicio_{{ $workingHours->id }}">Hora de apertura:</label>
                                                            <input type="text" class="form-control timepicker hora_inicio"
                                                                name="hora_inicio" id="hora_inicio_{{ $workingHours->id }}"
                                                                value="{{ \Carbon\Carbon::parse($workingHours->hora_inicio)->format('h:i A') }}">
                                                        </div>
                                                        <div class="form-group">
                                                            <label for="hora_fin_{{ $workingHours->id }}">Hora de cierre:</label>
                                                            <input type="text" class="form-control timepicker hora_fin"
                                                                name="hora_fin" id="hora_fin_{{ $workingHours->id }}"
                                                                value="{{ \Carbon\Carbon::parse($workingHours->hora_fin)->format('h:i A') }}">
                                                        </div>
                                                        <div class="form-group">
                                                            <label for="laborable_{{ $workingHours->id }}">Actividad del día</label>
                                                            <select class="form-control" name="laborable" id="laborable_{{ $workingHours->id }}">
                   ...
                   <!-- Modal para crear horario laboral -->
<div class="modal fade" id="createModal" tabindex="-1" role="dialog" aria-labelledby="createModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form id="createFormWorkingHours" action="{{ route('seller.business_working_hours.store') }}" method="POST" class="working-hours-form">
                @csrf
                <div class="modal-body">
                    <div class="form-group">
...
<!-- Script para manejar flatpickr y conversión de hora -->
...
<script>
   document.addEventListener('DOMContentLoaded', function () {
    ...
    function initializeFlatpickrr() {
        let timepickers = document.querySelectorAll('#createModal .timepicker');
    ...
    $('#createModal').on('shown.bs.modal', function () {
        initializeFlatpickrr();
    });

    // Manejar el envío del formulario
    document.getElementById('createFormWorkingHours').addEventListener('submit', function (e) {
        e.preventDefault();
        ...
        if (!horaInicio || !horaFin) {
            console.error('ERROR No se encontraron los campos de hora en createModal.');
            return;
        }

        const originalInicio = horaInicio.value;
        const originalFin = horaFin.value;

        horaInicio.value = convertTo24Hour(horaInicio.value);
        horaFin.value = convertTo24Hour(horaFin.value);

        this.submit();

        // Restaurar valores originales para evitar confusión visual
        horaInicio.value = originalInicio;
        horaFin.value = originalFin;
    });
});

    </script>

<!-- Script para manejar la conversión de horas -->
<script>
    document.addEventListener('DOMContentLoaded', function() {
    function convertTo24Hour(time12) {
        if (!time12) return '';
    ...
    function convertTo12Hour(value) {
        if (!value || value.includes('AM') || value.includes('PM')) return value;
        let [hours, minutes] = value.split(':').map(num => parseInt(num) || 0);
    ...
    function initializeFlatpickr(input) {
        let currentPeriod = input.value.includes('PM') ? 'PM' : 'AM';
        flatpickr(input, {
            enableTime: true,
            noCalendar: true,
            dateFormat: "h:i K",
            ...
            onClose: function(selectedDates, dateStr, instance) {
                if (dateStr && !dateStr.includes('AM') && !dateStr.includes('PM')) {
                    instance.input.value = dateStr + ' ' + currentPeriod;
                } else if (dateStr) {
                    currentPeriod = dateStr.includes('PM') ? 'PM' : 'AM';
                }
            },
    ...
    // Configurar Flatpickr para todos los inputs de hora
    document.querySelectorAll('.timepicker').forEach(initializeFlatpickr);

    // Interceptar el envío del formulario para convertir a formato 24h
    document.querySelectorAll('.working-hours-form').forEach(form => {
        form.addEventListener('submit', function(e) {
            e.preventDefault();

            const formId = this.dataset.id;
            const horaInicio = document.getElementById(`hora_inicio_${formId}`);
            const horaFin = document.getElementById(`hora_fin_${formId}`);

            if (!horaInicio || !horaFin) {
                console.error(`Error No se encontraron los campos de hora para el formulario con ID ${formId}.`);
                return;
            }
            ...
            console.log('Enviando formulario con ID:', formId);
            console.log('Hora de inicio (24h):', horaInicio.value);
            console.log('Hora de fin (24h):', horaFin.value);
