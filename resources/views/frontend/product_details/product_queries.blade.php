@if (get_setting('product_query_activation') == 1)
    <div class="bg-white border mt-4 mb-4 br-banner" id="product_query">
        ...
        <!-- Login & Register -->
        @guest
            <p class="fs-14 fw-400 mb-0 px-3 px-sm-4 mt-3"><a href="{{ route('user.login') }}">{{ translate('Login') }}</a>
                {{ translate('or') }} <a class="mr-1"
            ...
            <div class="query form px-3 px-sm-4">
                ...
                <form action="{{ route('product-queries.store') }}" method="POST">
                    ...
                    <div class="form-group">
                        <textarea class="form-control rounded-15px" rows="3" cols="40" name="question"
                            placeholder="{{ translate('Write your question here...') }}" style="resize: none;"></textarea>

                    </div>
                    <button type="submit"
                        class="btn btn-sm w-150px btn-primary rounded-25px">{{ translate('Submit') }}</button>
                </form>
            </div>
