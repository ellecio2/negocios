                    ...
                    @foreach ($bids as $key => $bid_id)
                        ...
                        <tr>
                            <td class="pl-0" style="vertical-align: middle;">
                                {{ sprintf('%02d', $key + 1 + ($bids->currentPage() - 1) * $bids->perPage()) }}</td>
                            ...
                            <td style="vertical-align: middle;">
                                ...
                                <span
                                    class="badge badge-inline @if ($bid->amount < $highest_bid) badge-danger @else badge-success @endif p-3 fs-12"
                            ...
                            <td class="text-right pr-0" style="vertical-align: middle;">
                                @php
                                    ...
                                    if ($order_detail != null) {
                                        $order = \App\Models\Order::where('id', $order_detail->order_id)
                                ...
                                @if ($bid->product->auction_end_date < strtotime('now') && $bid->amount == $highest_bid && $order == null)
                                    ...
                                    @if (count($carts) > 0)
                                        ...
                                        @if ($cart_has_this_product)
                                            <button type="button" class="btn btn-sm btn-primary buy-now fw-600 rounded-0"
                                                data-toggle="tooltip"
                                        ...
                                        @else
                                            <button type="button" class="btn btn-sm btn-primary buy-now fw-600 rounded-0"
                                                data-toggle="tooltip"
                                                title="{{ translate('Remove other items to add auction product.') }}">
                                                {{ translate('Buy') }}
                                            </button>
                                        @endif
                                    @else
                                    ...
                                    @endif
                                @elseif($order != null)
                                    <span class="badge badge-success p-3 fs-12"
                                        style="border-radius: 25px; min-width: 80px !important;">{{ translate('Purchased') }}</span>
                                @else
                                    N\A
                                @endif
                            </td>
