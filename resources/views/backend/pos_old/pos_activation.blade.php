                    ...
                    <form class="form-horizontal" action="{{ route('business_settings.update') }}" method="POST">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="input-group mb-3">
                                <input type="text" class="form-control" style="border-radius: 15px 0 0 15px"
                                    name="print_width" placeholder="{{ translate('Print width in mm') }}"
                                    value="{{ get_setting('print_width') }}">
                                <div class="input-group-append">
                                    <span class="input-group-text" style="border-radius:0 15px 15px 0"
                                        id="basic-addon2">{{ translate('mm') }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="form-group mb-0 text-right">
                            <button type="submit"
                                class="btn btn-sm btn-primary rounded-25px">{{ translate('Save') }}</button>
                        </div>
                    </form>
                </div>
            </div>
