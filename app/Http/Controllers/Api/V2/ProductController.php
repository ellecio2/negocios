...
class ProductController extends Controller
{
    ...
    public function category($id, Request $request)
    {
        ...
        $products->where('published', 1);
        return new ProductDetailCollection(filter_products($products)->latest()->paginate(10));
    }
    ...
    $products = Product::where('featured', 1)
        ...
        ->with(['category', 'brand', 'stocks'])
        ->select([
            'id', 'name', 'thumbnail_img', 'photos', 'unit_price', 'purchase_price', 'variant_product',
            'description', 'attributes', 'discount', 'discount_type', 'discount_start_date', 'discount_end_date',
            'rating', 'num_of_sale', 'slug', 'category_id', 'brand_id', 'current_stock', 'unit', 'min_qty',
            'low_stock_quantity', 'created_at', 'user_id'
        ])
        ->latest()
        ->paginate(24);

    // Usar la colección de recursos
    $productCollection = new ProductMiniCollection($products);

    // Puedes modificar la respuesta para agregar campos extra si lo necesitas
    $data = $productCollection->toResponse(request())->getData();

    // Ejemplo: agregar el slug a cada producto (como en bestSeller)
    foreach ($data->data as $key => $product) {
        $currentProduct = $products[$key];
        $data->data[$key]->slug = $currentProduct->slug;
        // Puedes agregar aquí shop u otros campos si tu Resource no los incluye
    }
...
//                     // Agrega aquí otros campos relevantes del shop
//                 ] : null;
//
//                 return [
//                     'id' => $product->id,
...
//                         'details' => route('product', $product->slug)
//                     ],
//                     'shop' => $shopDetails, // <--- Aquí agregas los detalles del shop
//                 ];
//             }),
//             'meta' => [
//                 'current_page' => $products->currentPage(),
...
//         ]);
    ...
    public function bestSeller()
    {
        ...
        // Obtener la colección estándar
        $productCollection = new ProductDetailCollection($filteredProducts);
    ...
    public function getProductsByCitieId($citieId, Request $request)
    {
        // Find shops located in the specified state
        $shops = Shop::where('city', $citieId)->get();

        // Extract user IDs from the shops
        $userIds = $shops->pluck('user_id');

        // Query products from these shops
        $products = Product::whereIn('user_id', $userIds)->where('published', 1);
        ...
        return new ProductDetailCollection($products->latest()->paginate(10));
    }

    public function getProductsByStateId($stateId, Request $request)
    {
        // Find shops located in the specified state
        $shops = Shop::where('state', $stateId)->get();
    ...
    public function getProductsByCountryId($countryId, Request $request)
    {
        // Find shops located in the specified state
        $shops = Shop::where('country', $countryId)->get();
...
public function frequentlyBoughtTogether(Request $request, $id)
{
    // Validar que el producto existe
    $product = Product::findOrFail($id);
    
    // Parámetros opcionales con valores por defecto
    $limit = $request->input('limit', 5); // Número de productos a mostrar
    $minOccurrences = $request->input('min_occurrences', 2); // Mínimo de veces que deben aparecer juntos
    
    // Cache key único basado en parámetros
    $cacheKey = 'frequently_bought_together_' . $id . '_' . $limit . '_' . $minOccurrences;
    $cacheDuration = 60 * 24; // 24 horas en minutos
    
    // Obtener resultados usando cache
    $relatedProducts = Cache::remember($cacheKey, $cacheDuration, function () use ($id, $limit, $minOccurrences) {
        ...
        // Encontrar otros productos en esas mismas órdenes (excluyendo el producto actual)
        $frequentlyBoughtProducts = OrderDetail::whereIn('order_id', $orderIds)
            ->where('product_id', '!=', $id)
            ->select('product_id', DB::raw('COUNT(*) as occurrence_count'))
            ->groupBy('product_id')
        ...
        return Product::whereIn('id', $frequentlyBoughtProducts)
                ->where('published', 1) // Solo productos publicados
                ->get();
    });
